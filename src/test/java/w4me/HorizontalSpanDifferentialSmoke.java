...
public final class HorizontalSpanDifferentialSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        module.close();
        System.out.println(
                "PASS horizontal-span differential cases="
                        + cases
                        + " colors=4 alignments=all boundaries=exact");
    }
        ...
        for (index = 0; index < expected.length; index++) {
            ...
            if (found != wanted) {
                throw new AssertionError(
                        "horizontal span mismatch color="
                                + color
                                + " start="
                                + start
                                + " y="
    ...
    private static void drawReference(
            byte[] framebuffer, int color, int start, int y, int length) {
