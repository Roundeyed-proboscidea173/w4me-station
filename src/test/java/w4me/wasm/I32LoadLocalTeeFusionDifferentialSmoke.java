...
public final class I32LoadLocalTeeFusionDifferentialSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        if (arguments.length != 2) {
            throw new IllegalArgumentException(
                    "usage: font.bin i32-load-local-tee-fusion.wasm");
        }
        byte[] font = readFile(arguments[0]);
        byte[] cartridge = readFile(arguments[1]);

        Result outerReference =
                run(font, cartridge, "update", 200000000L, false, false);
        Result outerCandidate =
                run(font, cartridge, "update", 200000000L, true, false);
        ...
        if (outerReference.fusedSites != 0 || outerCandidate.fusedSites != 3) {
            throw new AssertionError(
                    "unexpected fused site count: reference="
        ...
        if (outerReference.dispatches - outerCandidate.dispatches != 2) {
            throw new AssertionError(
                    "expected two saved outer dispatches: reference="
                            + outerReference.dispatches
                            + " candidate="
                            + outerCandidate.dispatches);
        }

        Result compactReference =
                run(font, cartridge, "update", 200000000L, false, true);
        Result compactCandidate =
                run(font, cartridge, "update", 200000000L, true, true);
        assertEquivalent("compact-success", compactReference, compactCandidate);
        if (compactReference.compactInstructions == 0
                || compactCandidate.compactInstructions == 0) {
            throw new AssertionError("focused pair did not reach compact execution");
        }

        long firstBudget = outerReference.instructions - 25;
        long budget;
        for (budget = firstBudget;
                budget <= outerReference.instructions + 1;
                budget++) {
            assertEquivalent(
                    "outer-budget-" + budget,
        ...
        System.out.println(
                "PASS i32-load-local-tee-fusion sites=3 outer-dispatches-saved=2"
                        + " logical="
    ...
    private static Result run(
            ...
            throws Exception {
        WasmModule module =
                WasmModule.read(cartridge, null, true, loadTeeFusions);
    ...
    private static int countFusedSites(WasmModule module) {
        int count = 0;
        int function;
        for (function = module.imports.length;
                function < module.functions.length;
                function++) {
            WasmModule.FunctionBody body = module.functions[function];
            int instruction;
            for (instruction = 0;
                    instruction < body.instructionCount();
                    instruction++) {
                int opcode =
                        WasmModule.originalOpcode(
                                body.code[instruction * WasmModule.W4IR_STRIDE]
                                        & 0xffff);
    ...
    private static void assertEquivalent(
            String label, Result reference, Result candidate) {
        requireEquals(label, "trap", reference.trap, candidate.trap);
        requireEquals(
                label,
                "instructions",
                Long.toString(reference.instructions),
    ...
    private static void compareBytes(
            String label, String field, byte[] left, byte[] right) {
        ...
        for (index = 0; index < left.length; index++) {
            if (left[index] != right[index]) {
                throw new AssertionError(
                        label + " " + field + " mismatch at " + index);
            }
        }
    }

    private static void compareLongs(
            String label, String field, long[] left, long[] right) {
        if (left.length != right.length) {
            throw new AssertionError(label + " " + field + " length mismatch");
        }
        int index;
    ...
    private static void compareInts(
            String label, String field, int[] left, int[] right) {
        if (left.length != right.length) {
            throw new AssertionError(label + " " + field + " length mismatch");
        }
        int index;
