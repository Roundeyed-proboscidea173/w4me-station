...
public final class Wasm4PcmDifferentialSmoke {
    ...
    private static void check(
            int frequency, int duration, int volume, int flags) {
        byte[] expected =
                referenceSynthesize(frequency, duration, volume, flags);
        byte[] actual =
                Wasm4Pcm.synthesize(frequency, duration, volume, flags);
     ...
     */
    private static byte[] referenceSynthesize(
            int frequency, int duration, int volume, int flags) {
        ...
        int sustainVolume = referenceClamp(volume & 0xff, 0, 100);
        ...
        int startFrequency =
                referenceDecodeFrequency(frequency & 0xffff, noteMode);
        int endFrequency =
                referenceDecodeFrequency(
                        (frequency >>> 16) & 0xffff, noteMode);
        if (endFrequency == 0) {
            endFrequency = startFrequency;
        }
        boolean peakAudible =
                peakVolume > 0 && (attack > 0 || decay > 0);
        boolean sustainAudible =
                sustainVolume > 0 && (sustain > 0 || release > 0);
        if (totalFrames == 0
                || startFrequency <= 0
                || (!peakAudible && !sustainAudible)) {
            return null;
        }
    ...
    private static int referenceEnvelopeVolume(
        ...
        if (elapsed < attack) {
            return attack == 0
                    ? peakVolume
                    : peakVolume * elapsed / attack;
        }
        elapsed -= attack;
        if (elapsed < decay) {
            return peakVolume
                    + (sustainVolume - peakVolume) * elapsed / decay;
        }
        elapsed -= decay;
        if (elapsed < sustain) {
        ...
        if (elapsed < release) {
            return release == 0
                    ? 0
                    : sustainVolume * (release - elapsed) / release;
        }
        return 0;
    }

    private static int referenceDecodeFrequency(
            int encoded, boolean noteMode) {
        ...
        double semitones =
                (double) note - 69.0 + (double) bend / 256.0;
        return (int)
                Math.floor(
                        440.0
                                        * CldcMath.powerOfTwo(
    ...
    private static void referenceWriteWavHeader(
            byte[] wav, int dataLength, int channels) {
    ...
    private static void referenceWriteAscii(
            byte[] data, int offset, String value) {
        int index;
    ...
    private static void referenceWriteShortLe(
            byte[] data, int offset, int value) {
        data[offset] = (byte) value;
        data[offset + 1] = (byte) (value >>> 8);
    }

    private static void referenceWriteIntLe(
            byte[] data, int offset, int value) {
        data[offset] = (byte) value;
        data[offset + 1] = (byte) (value >>> 8);
    ...
    private static int referenceClamp(
    ...
    private static void assertEquals(
            String label, long expected, long actual) {
        if (expected != actual) {
            throw new AssertionError(
