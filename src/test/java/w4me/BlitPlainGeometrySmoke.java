...
public final class BlitPlainGeometrySmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        verifyCase(
                ...
                0xffff,
                caseCount++);
        verifyOverlap(runtime, module, 0, caseCount++);
        verifyOverlap(runtime, module, 1, caseCount++);
        verifyDestructiveOverlap(runtime, module, 0, caseCount++);
        verifyDestructiveOverlap(runtime, module, 1, caseCount++);
        verifyOffsetDestructiveOverlap(runtime, module, 0, caseCount++);
        verifyOffsetDestructiveOverlap(runtime, module, 1, caseCount++);
        verifyBlitDelegation(runtime, module, caseCount++);
        verifyNoOpAndTraps(runtime, module);
        runtime.close();

        System.out.println(
                "PASS blit-plain-geometry cases="
                        + caseCount
    ...
    private static void verifyOverlap(
            Wasm4Runtime runtime, WasmModule module, int flags, int salt) {
    ...
    private static void verifyBlitDelegation(
            Wasm4Runtime runtime, WasmModule module, int salt) {
        byte[] actual = module.memory();
        initialize(actual, salt);
        ...
        referenceBlitSub(
                ...
                1);
        long[] values = {
            SOURCE_POINTER, -3, 7, width, height, 1
        };
        runtime.invoke(
                WasmHost.IMPORT_BLIT,
                values,
                0,
                values.length,
                module);
    ...
    private static void verifyDestructiveOverlap(
            Wasm4Runtime runtime, WasmModule module, int flags, int salt) {
    ...
    private static void verifyOffsetDestructiveOverlap(
            Wasm4Runtime runtime, WasmModule module, int flags, int salt) {
    ...
    private static void verifyNoOpAndTraps(
            Wasm4Runtime runtime, WasmModule module) {
        ...
        expectTrap(
                runtime,
                module,
                new long[] {SOURCE_POINTER, 0, 0, 1, 1, -1, 0, 1, 0},
                "invalid blit source geometry");
        expectTrap(
                runtime,
                module,
                new long[] {SOURCE_POINTER, 0, 0, 1, 1, 0, 0, 0, 1},
                "invalid blit source geometry");
        expectTrap(
                runtime,
                module,
    ...
    private static void referenceBlitSub(
        ...
        if (pointer < 0
                || byteLength < 0
                || byteLength > memory.length - pointer) {
        ...
        int clipXMaximum =
                clampToRange(Wasm4Runtime.HEIGHT - clipXOrigin, width);
        int clipYMaximum =
                clampToRange(Wasm4Runtime.WIDTH - clipYOrigin, height);
        ...
        for (yIndex = clipYMinimum; yIndex < clipYMaximum; yIndex++) {
            ...
            for (xIndex = clipXMinimum; xIndex < clipXMaximum; xIndex++) {
                int targetX =
                        destinationX + (rotate ? yIndex : xIndex);
                int targetY =
                        destinationY + (rotate ? xIndex : yIndex);
                int sampledX =
                        sourceX + (flipX ? width - xIndex - 1 : xIndex);
                int sampledY =
                        sourceY + (flipY ? height - yIndex - 1 : yIndex);
    ...
    private static void drawPoint(
            byte[] memory, int color, int x, int y) {
    ...
    private static void requireMemory(
            String label, byte[] expected, byte[] actual) {
        int index;
