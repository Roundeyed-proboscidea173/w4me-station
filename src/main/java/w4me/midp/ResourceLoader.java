...
final class ResourceLoader {
    ...
    static int stage(CartridgeStore store, String title, String location)
        ...
        if (startsWithIgnoreCase(location, "file://")
                && FileSystemAccessFactory.isAvailable()) {
            return stageFile(
                    store, title, location, FileSystemAccessFactory.create());
        }
    ...
    static int stageFile(
        ...
        return store.stageStream(
                title, access.openInputStream(selection.url), selection.size);
    }
    ...
    private static byte[] readInstalled(String location) throws IOException {
        ...
        try {
            ...
            throw new IOException("cannot read installed cartridge: " + failure.toString());
        } finally {
            if (store != null) {
                store.close();
            }
            ...
            while ((count = input.read(buffer)) >= 0) {
                ...
                if (total > CartridgeStore.MAX_CARTRIDGE_BYTES) {
                    throw new IOException(
                            "cartridge exceeds the WASM-4 64 KiB limit: " + location);
                }
